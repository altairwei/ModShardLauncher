...
/// 新契约：BeginBuild 在 boot 期（OnInitGML）后台构建；hello 每连接快照索引现状。</summary>
public class IndexBuildTests : IDisposable
{
    ...
    [Fact]
    public void BeginBuild_SmallIndex_FailsStatusOnBuildThread()
    {
        PlantNode(0x10100, 0x10C00, "test_entry");   // 1 节点 < 30000 → 规模护栏在构建线程上报
        NodeIndex.BeginBuild();
